...
import { SignUpDataSchema, SignUpDataType, UserSchema } from "@/lib/types";
...
export default function SignUpDialogContent() {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<SignUpDataType>({ resolver: zodResolver(SignUpDataSchema) });

  const onSubmit = handleSubmit(async (formData) => {
    startTransition(async () => {
      const signedup = await emailSignUp(formData, setUserData);
      if (signedup) {
